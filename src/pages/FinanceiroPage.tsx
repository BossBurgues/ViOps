...
import { DollarSign, AlertTriangle, CheckCircle, Clock, TrendingUp, CreditCard, Search, Shield, Lock, FileDown, FileSpreadsheet, Users } from 'lucide-react';
...
type FinView = 'parcelas' | 'por_cliente' | 'por_unidade';
...
export default function FinanceiroPage() {
  ...
  const aVencer = pendentes.filter(p => p.vencimento >= '2025-04-12');
  ...
  const byClient = [...new Set(allParcelas.map(p => p.clienteId))].map(cid => {
    ...
    const cpVenc = cpPend.filter(p => p.vencimento < '2025-04-12');
  ...
  const byUnit = unidades.filter(u => u.ativa).map(u => {
    ...
    const upVenc = upPend.filter(p => p.vencimento < '2025-04-12');
    return {
  ...
  const handleExportCSV = () => {
    ...
    const rows = displayParcelas.map(p => {
      ...
      return [p.osNumero, p.clienteNome, p.formaPagamento, p.numero, p.vencimento, p.valor.toFixed(2), p.status === 'paga' ? 'Paga' : isOverdue ? 'Vencida' : 'Pendente', p.osStatus];
    });
    ...
    <AppLayout>
      <div className="space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-6">
          ...
          <KpiCard title="Inadimplencia" value={`${inadimplenciaRate}%`} icon={TrendingUp} />
        ...
        <div className="flex items-center gap-1.5 border-b border-border pb-0">
          {([['parcelas', 'Parcelas'], ['por_cliente', 'Por Cliente'], ['por_unidade', 'Por Unidade']] as const).map(([key, label]) => (
            <button
              key={key}
              onClick={() => setView(key)}
                          ...
                          return (
                            <tr key={p.id} className={`border-b border-border/40 last:border-0 transition-colors hover:bg-muted/40 ${isOverdue ? 'bg-destructive/[0.02]' : ''}`}>
                              ...
                              <td className="px-5 py-3">
                                <span className={`status-badge ${p.status === 'paga' ? 'status-pronta' : isOverdue ? 'status-cancelada' : 'status-pendencia'}`}>
        ...
        <div className="page-card px-6 py-4 flex items-center gap-3 border-dashed">
          ...
          <div>
            <p className="text-[13px] font-medium text-foreground">Integracao Bancaria</p>
            <p className="text-[12px] text-muted-foreground">Modulo preparado para conciliacao automatica com bancos e gateways de pagamento.</p>
          </div>
        </div>
      </div>
